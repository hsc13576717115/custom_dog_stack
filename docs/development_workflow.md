...
当前四个 calf 的几何硬限位为 `[-162 deg, -48 deg]`，用于表达已测得的折叠/趴地
姿态。训练资产仍使用 `soft_joint_pos_limit_factor=0.9`，因此 RL 的软安全区约为
`[-156.3 deg, -53.7 deg]`。硬限位不是可长期下发的实机目标；起身控制必须使用单独的
软件安全限位、低增益和悬空测试，且现有行走策略没有接受趴地姿态训练。
