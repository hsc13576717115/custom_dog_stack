...
task="CustomDog-Velocity-ClosedLoop-History213-Distill-v1"
...
if [[ ${#available[@]} -lt 5 ]]; then
    echo "Expected at least five history-distillation checkpoints, found ${#available[@]}" >&2
    exit 1
fi
...
candidate_args=(--candidate "teacher=${source_candidate}")
terrain_args=()
for checkpoint in "${checkpoints[@]}"; do
    iteration="${checkpoint#model_}"
    iteration="${iteration%.pt}"
    label="H213_${iteration}"
    ...
    terrain_result="${run_dir}/evaluation/${label}_terrain_t1.json"
    if OMNI_KIT_ACCEPT_EULA=YES python \
        ...
        echo "${label}: deployable terrain gate passed"
    else
        echo "${label}: deployable terrain gate failed"
    fi
